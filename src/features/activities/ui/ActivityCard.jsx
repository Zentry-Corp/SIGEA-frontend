...
const ActivityCard = ({
  ...
  const sesiones = Array.isArray(raw)
    ? raw
    : Array.isArray(raw?.extraData)
    ? raw.extraData
    : [];
  const loadingSesiones = Boolean(
    sessionsResult?.loading ?? sessionsResult?.isLoading ?? false,
  );
  ...
  return (
    <Card $accentColor={estadoConfig.color}>
      ...
      <CardFooter>
        <ActionButton
          onClick={() => onViewDetail && onViewDetail(activity)}
          $variant="primary"
